"""Preprocess 씨마스 고등학교 스포츠 문화 교과서 PDF into page/spread search assets.

Same engine as preprocess_elementary_pe_2022_more_publishers.py, but the manifest
matches the existing high-school sibling documents' schema (22개정_donga_textbook,
22개정_chunjae_textbook, etc.) rather than the elementary grade/curriculum_band
schema, since this is a 고등학교 "스포츠 문화" textbook, not an elementary PE book.

Note: the sibling high-school documents (donga/chunjae/miraen/jihaksa) were produced
by a real gutter/color-continuity spread classifier whose source script is no longer
in this repo (only its output survives in their spreads.jsonl). This script instead
reuses the simpler sequential-pairing-as-"uncertain" approach used for every document
processed today, so this document's classification is honest-but-simpler than its
siblings' until/unless that classifier is rebuilt.
"""
...
def preprocess(source: Path, output: Path, document_id: str, publisher: str, document_type: str, source_file: str) -> None:
    if output.exists():
...
def main() -> None:
    ...
    rel_path = "textbook/씨마스 고등학교 스포츠 문화.pdf"
    document_id = "22개정_ssimas_textbook"
    source = root / "official-data" / rel_path
    output = root / "official-data" / "processed" / document_id
    if not source.exists():
        raise FileNotFoundError(source)
    if output.exists():
        print(f"[{document_id}] 이미 처리됨", flush=True)
        return
    preprocess(source, output, document_id, "씨마스", "교과서", rel_path)
