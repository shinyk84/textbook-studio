...
const steps = [
  ...
  ["원고 편집", "샘플 원고를 직접 수정하고 저장 흐름을 확인합니다."],
];
...
function manuscriptEntryFor(option, approvedFramework) {
  if (!state.manuscriptBySmallUnit) state.manuscriptBySmallUnit = {};
  if (!state.manuscriptBySmallUnit[option.key]) {
    const metrics = state.frameworkMetrics?.[approvedFramework.id] || approvedFramework.metrics;
    const draft = simulatedManuscriptDraft(approvedFramework, metrics, option);
    state.manuscriptBySmallUnit[option.key] = {
      author: "",
      spreads: draft.spreads.map((spread) => ({
        intro: spread.intro,
        activity1: spread.activities[0] ? `${spread.activities[0].title}\n${spread.activities[0].objective}\n${spread.activities[0].method.join(" / ")}` : "",
        activity2: spread.activities[1] ? `${spread.activities[1].title}\n${spread.activities[1].objective}\n${spread.activities[1].method.join(" / ")}` : "",
        support: spread.support_boxes.map((box) => `[${box.type}] ${box.content}`).join("\n"),
        wrapUp: spread.wrap_up,
      })),
    };
  }
  return state.manuscriptBySmallUnit[option.key];
}
...
function renderFrameworks() {
  ...
  return `
    ${sectionHeading("FORMAT OPTIONS", "체제안 선택", "이번 버전에서는 설명과 예상 구조만 비교합니다. 승인한 체제는 5단계 원고 작성 전체에 적용됩니다.")}
    ...
    <div class="framework-approval-bar ${approval.status === "approved" ? "approved" : ""}">
      <div>
        <b>체제 승인</b>
        <span>${approval.status === "approved" ? `승인됨 · ${escapeHtml(approvedFramework?.name || "")} (5단계 원고 작성에 적용됨)` : "아직 승인된 체제가 없습니다. 아래에서 초안을 비교한 뒤 하나를 승인하세요."}</span>
      </div>
...
function renderManuscript() {
  const approval = state.frameworkApproval;
  const approvedFramework = approval?.status === "approved" ? frameworks.find((framework) => framework.id === approval.frameworkId) : null;
  if (!approvedFramework) {
    return `
      ${sectionHeading("MANUSCRIPT", "전체 원고 작성", "4단계에서 체제안을 승인하면 소단원별 초고 작성 화면이 열립니다.")}
      <div class="validation-box invalid"><strong>아직 승인된 체제안이 없습니다.</strong><span>4단계(체제안 선택)에서 초안을 비교하고 하나를 승인해 주세요.</span></div>`;
  }
  const options = smallUnitOptions();
  return `
    ${sectionHeading("MANUSCRIPT", "전체 원고 작성", `승인된 체제: ${approvedFramework.name}. 소단원마다 담당 집필자를 지정하고 초고를 작성합니다. 모든 입력은 브라우저에 자동 저장됩니다.`)}
    <div class="manuscript-unit-list">
      ${options.map((option) => {
        const entry = manuscriptEntryFor(option, approvedFramework);
        return `
        <details class="manuscript-unit-card">
          <summary>
            <span class="domain-badge ${option.domain}">${option.domain}</span>
            <b>${escapeHtml(option.middleTitle)} · ${escapeHtml(option.smallTitle)}</b>
            <small>${entry.author ? `담당: ${escapeHtml(entry.author)}` : "담당자 미지정"}</small>
          </summary>
          <label class="editor-field"><span>담당 집필자</span><input data-manuscript-author="${option.key}" value="${escapeHtml(entry.author)}" placeholder="이름 입력" /></label>
          ${entry.spreads.map((spread, spreadIndex) => `
            <div class="manuscript-spread">
              <p class="spread-field-label">본문</p>
              <textarea data-manuscript-field="${option.key}:${spreadIndex}:intro">${escapeHtml(spread.intro)}</textarea>
              <p class="spread-field-label">활동 1</p>
              <textarea data-manuscript-field="${option.key}:${spreadIndex}:activity1">${escapeHtml(spread.activity1)}</textarea>
              <p class="spread-field-label">활동 2</p>
              <textarea data-manuscript-field="${option.key}:${spreadIndex}:activity2">${escapeHtml(spread.activity2)}</textarea>
              <p class="spread-field-label">기타 보조단</p>
              <textarea data-manuscript-field="${option.key}:${spreadIndex}:support">${escapeHtml(spread.support)}</textarea>
              <p class="spread-field-label">마무리</p>
              <textarea data-manuscript-field="${option.key}:${spreadIndex}:wrapUp">${escapeHtml(spread.wrapUp)}</textarea>
            </div>`).join("")}
        </details>`;
      }).join("")}
    </div>`;
...
function renderWorkspace() {
  const renderers = [renderProject, renderCurriculum, renderUnits, renderFrameworks, renderManuscript];
  const project = activeProject();
...
function bindWorkspace() {
  ...
  document.querySelectorAll("[data-approve-framework]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      persist("체제 승인됨");
      renderWorkspace();
      showToast("이 체제로 승인했습니다. 5단계에서 전체 원고를 작성할 수 있습니다.");
    });
  });
  ...
  document.querySelectorAll("[data-manuscript-author]").forEach((input) => {
    input.addEventListener("input", () => {
      const key = input.dataset.manuscriptAuthor;
      state.manuscriptBySmallUnit[key].author = input.value;
      persist();
    });
  });

  document.querySelectorAll("[data-manuscript-field]").forEach((textarea) => {
    textarea.addEventListener("input", () => {
      const parts = textarea.dataset.manuscriptField.split(":");
      const field = parts.pop();
      const spreadIndex = Number(parts.pop());
      const key = parts.join(":");
      state.manuscriptBySmallUnit[key].spreads[spreadIndex][field] = textarea.value;
      persist();
    });
  });
...
function goToStep(index) {
  ...
  if (state.currentStep === 3 && nextIndex > 3 && state.frameworkApproval?.status !== "approved") {
    showToast("체제안을 승인한 뒤 5단계로 이동할 수 있습니다.");
    return;
  }
...
document.querySelector("#nextButton").addEventListener("click", () => {
  ...
  if (state.currentStep === 3 && state.frameworkApproval?.status !== "approved") {
    showToast("체제안을 승인한 뒤 5단계로 이동할 수 있습니다.");
    return;
  }
